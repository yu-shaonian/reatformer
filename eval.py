...
parser.add_argument('--loadckpt', default="/mnt/nas_8/datasets/guojun/fix_decode_v100/model_000015.ckpt",
                    help='load a specific checkpoint')
parser.add_argument('--outdir', default='/data-1/leiguojun/mvs_output/mvs_decode_fix', help='output dir')
...
parser.add_argument('--max_h', type=int, default=543, help='testing max h')
parser.add_argument('--max_w', type=int, default=640, help='testing max w')
parser.add_argument('--fix_res', action='store_true', help='scene all using same res')
parser.add_argument('--num_worker', type=int, default=5, help='depth_filer worker')
...
#filter by gimupa
parser.add_argument('--fusibile_exe_path', type=str, default='/home/leiguojun/source_code/fusibile/fusibile')
...
def save_scene_depth(testlist):
    ...
    with torch.no_grad():
        for batch_idx, sample in enumerate(TestImgLoader):
            ...
            import ipdb
            ipdb.set_trace()
            outputs = model(sample_cuda["imgs"], sample_cuda["proj_matrices"]["down1"], sample_cuda["depth_values"])
            depth_est = outputs["depth"]
            end_time = time.time()
            outputs = tensor2numpy(depth_est)
            del sample_cuda
            filenames = sample["filename"]
            cams = sample["proj_matrices"]["down1"].numpy()
            imgs = sample["imgs"].numpy()
            print('Iter {}/{}, Time:{} Res:{}'.format(batch_idx, len(TestImgLoader), end_time - start_time, imgs[0].shape))

            # save depth maps and confidence maps
            for filename, cam, img, depth_est in zip(filenames, cams, imgs, outputs):
                img = img[0]  #ref view
                cam = cam[0]  #ref cam
...
def filter_depth(pair_folder, scan_folder, out_folder, plyfilename):
    ...
    for ref_view, src_views in pair_data:
        ...
        print("processing {}, ref-view{:0>2}, geo/final-mask:{}/{}".format(scan_folder, ref_view,
...
if __name__ == '__main__':
    ...
    # #pcd_filter是采用normal的方式，下面是采用gipuma的方式
    ...
    mrun_rst(
            ...
            plyPath='/mnt/nas_8/datasets/guojun/mvs_decode_fix_lr'
        )
    ...
    #
    #     # step2. filter saved depth maps with photometric confidence maps and geometric constraints
    #     filter_depth(pair_folder, scan_folder, out_folder, os.path.join(args.outdir, 'mvsnet{:0>3}_l3.ply'.format(scan_id)))