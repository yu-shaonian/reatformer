...
parser.add_argument('--trainpath',default='/data-1/leiguojun/data/mvs_training/dtu/',help='train datapath')
...
parser.add_argument('--logdir', default='/data-1/leiguojun/logs/mvsformer/fix_decode_view5_hr_mvsformer_2', help='the directory to save checkpoints/logs')
...
def train():
    ...
    # lambda1 = lambda epoch: 0.1 ** epoch  # 学习率 = 0.1**(轮数)
    # lr_scheduler = optim.lr_scheduler.LambdaLR(optimizer, lr_lambda=lambda1)

    for epoch_idx in tqdm.tqdm(range(start_epoch, args.epochs)):
        global_step = len(TrainImgLoader) * epoch_idx

        # training
        loss = 0
        for batch_idx, sample in enumerate(tqdm.tqdm(TrainImgLoader, desc=f'Epoch: {epoch_idx:03d}/{args.epochs:03d}')):
...
def train_sample(sample, detailed_summary=False):
    ...
    # loss = model_loss(index_est, depth_gt, mask, sample_cuda["depth_values"])
    loss, depth_loss, entropy_loss, depth_entropy = model_loss(outputs, depth_gt, mask)
    loss.backward()
    optimizer.step()
...
def test_sample(sample, detailed_summary=True):
    ...
    # depth_est = model(sample_cuda["imgs"], sample_cuda["proj_matrices"], sample_cuda["depth_values"])
    outputs  = model(sample_cuda["imgs"], sample_cuda["proj_matrices"]["down1"], sample_cuda["depth_values"])
    depth_est = outputs["depth"]
