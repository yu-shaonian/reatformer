...
def initialize_flow(img):
    ...
    N, C1, C, H, W = img.shape
    # print('N, C, H, W测试，',N, C, H, W)
...
class CrossAttentionLayer(nn.Module):
    ...
    def forward(self, query, key, value, memory, query_coord, patch_size, size_h3w3):
        ...
        query_coord = query_coord.view(B, 1, -1).permute(0, 2, 1)[:,:,None,:].contiguous().view(B*H1*W1, 1)
        if self.pe == 'linear':
...
class MemoryDecoder(nn.Module):
    def __init__(self, cfg):
        ...
        self.flow_token_encoder = nn.Sequential(
            ...
            nn.Conv2d(9*cfg.cost_heads_num, dim, 1, 1),
            nn.GELU(),
            nn.Conv2d(dim, dim, 1, 1)
        )
        self.proj = nn.Conv2d(192, 192, 1)
        self.channel = nn.Conv2d(240, 9, 1)

        self.down_sample = nn.Conv2d(240, 9, 1)
        ...
        if self.cfg.gma:
            ...
            self.att = Attention(args=self.cfg, dim=64, heads=1, max_pos_size=160, dim_head=128)
        else:
            self.update_block = BasicUpdateBlock(self.cfg, hidden_dim=128)
    ...
    def encode_flow_token(self, cost_maps, coords):
        """
            cost_maps   -   B*H1*W1, cost_heads_num, M
            coords      -   B, 1, H1, W1
        """
        coords = coords.permute(0, 2, 3, 1)
        batch, h1, w1, _ = coords.shape
        coords = torch.cat([coords, torch.zeros_like(coords)], dim=-1)
        cost_maps = torch.unsqueeze(cost_maps, dim=-1)

        r = 4
        dy = torch.linspace(-r, r, 2*r+1)
        dx = torch.tensor(0, dtype=dy.dtype)
        delta = torch.stack(torch.meshgrid(dy, dx), axis=-1).to(coords.device)

        centroid = coords.reshape(batch*h1*w1, 1, 1, 2)
        delta = delta.view(1, 2*r+1, 1, 2)
        coords = centroid + delta
    ...
    def forward(self, cost_memory, context, data={}, flow_init=None):
        ...
        coords0, coords1 = initialize_flow(cost_memory)
        coords0 = coords0[:, 0:1]
        coords1 = coords1[:, 0:1]

        # context = self.proj(context)
        net, inp = torch.split(context, [128, 64], dim=1)
        net = torch.tanh(net)
        inp = torch.relu(inp)
        if self.cfg.gma:
        ...
        for idx in range(self.depth):
            ...
            # query其实就是cost_volume和ref img算了一些注意力得到的结果，这个是有用的
            ...
            flow_final = self.update_block(net, inp, corr, flow)
            B ,C ,H , W = flow_final.shape

            flow_up = torch.sum(flow_final.reshape(B, C*16, H//4,  W//4),dim=1)/(C*16)

            coords1 = coords1 + flow_up
