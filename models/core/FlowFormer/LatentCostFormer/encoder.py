...
from models.core.utils.utils import coords_grid, bilinear_sampler, upflow8
...
class PatchEmbed(nn.Module):
    def __init__(self, patch_size=16, in_chans=1, embed_dim=64, pe='linear'):
        ...
        if patch_size == 8:
            self.proj = nn.Sequential(
                nn.Conv1d(in_chans, embed_dim//4, kernel_size=6, stride=2, padding=2),
                nn.ReLU(),
                nn.Conv1d(embed_dim//4, embed_dim//2, kernel_size=6, stride=2, padding=2),
                nn.ReLU(),
                nn.Conv1d(embed_dim//2, embed_dim, kernel_size=6, stride=2, padding=2),
            )
        elif patch_size == 4:
            self.proj = nn.Sequential(
                nn.Conv1d(in_chans, embed_dim//4, kernel_size=6, stride=2, padding=2),
                nn.ReLU(),
                nn.Conv1d(embed_dim//4, embed_dim, kernel_size=6, stride=2, padding=2),
            )
        else:
    ...
    def forward(self, x) -> Tuple[torch.Tensor, Size_]:
        ...
        pad_l = 0
        pad_r = (self.patch_size - M % self.patch_size) % self.patch_size
        x = F.pad(x, (pad_l, pad_r))

        x = self.proj(x)
        out_size = x.shape[2:] 

        patch_coord = torch.arange(out_size[0]).to(x.device) * self.patch_size + self.patch_size/2
        patch_coord = patch_coord[None].repeat(B, 1)
        if self.pe == 'linear':
        ...
        patch_coord_enc = patch_coord_enc.permute(0, 2, 1).view(B, -1, out_size[0])
...
class CostPerceiverEncoder(nn.Module):
    ...
    def forward(self, cost_volume, data, context=None):
        ...
        B, heads, M, H1, W1 = cost_volume.shape
        cost_maps = cost_volume.permute(0, 3, 4, 1, 2).contiguous().view(B*H1*W1, self.cfg.cost_heads_num, M)
        data['cost_maps'] = cost_maps

        # print('--------------------')
        # print('cost_map:',cost_maps.shape)
...
class MemoryEncoder(nn.Module):
    def __init__(self, cfg1):
        ...
        self.feat_encoder = BasicEncoder(output_dim=192, norm_fn='instance')
    ...
    def corr(self, fmap1, fmap2):
        ...
        fmap1 = rearrange(fmap1, 'b (heads d) h w -> b heads (h w) d', heads=1)
        fmap2 = rearrange(fmap2, 'b (heads d) h w -> b heads (h w) d', heads=1)
        corr = einsum('bhid, bhjd -> bhij', fmap1, fmap2)
        corr = corr.permute(0, 2, 1, 3).view(batch*ht*wd, 1, ht, wd)
        #corr = self.norm(self.relu(corr))
        corr = corr.view(batch, ht*wd, 1, ht*wd).permute(0, 2, 1, 3)
