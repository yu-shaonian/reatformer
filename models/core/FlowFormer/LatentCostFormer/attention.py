...
def LinearPositionEmbeddingSine(x, dim=128, NORMALIZE_FACOR=1/200):
    ...
    freq_bands = torch.linspace(0, dim//2-1, dim//2).to(x.device)
    return torch.cat([torch.sin(3.14*x[...,None]*freq_bands*NORMALIZE_FACOR), torch.cos(3.14*x[...,None]*freq_bands*NORMALIZE_FACOR)], dim=-1)
...
def ExpPositionEmbeddingSine(x, dim=128, NORMALIZE_FACOR=1/200):
    ...
    freq_bands = torch.linspace(0, dim//2-1, dim//2).to(x.device)
    return torch.cat([torch.sin(x[...,None]*(NORMALIZE_FACOR * 2 ** freq_bands)), torch.cos(x[...,None]*(NORMALIZE_FACOR * 2 ** freq_bands))], dim=-1)