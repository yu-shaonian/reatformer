...
def depth_wta(p, depth_values):
    '''Winner take all.'''
    wta_index_map = torch.argmax(p, dim=1).type(torch.long)
    depth_value = depth_values.squeeze(0)
    wta_depth_map = depth_value[wta_index_map]
...
class FeatureNet(nn.Module):
    def __init__(self, base_channels):
        ...
        self.conv1 = nn.Sequential(
                Conv2d(base_channels, base_channels * 2, 3, stride=1, padding=1),
                Conv2d(base_channels * 2, base_channels * 2, 3, 1, padding=1),
                Conv2d(base_channels * 2, base_channels * 2, 3, 1, padding=1))

        self.conv2 = nn.Sequential(
                Conv2d(base_channels * 2, base_channels * 4, 3, stride=1, padding=1),
                Conv2d(base_channels * 4, base_channels * 4, 3, 1, padding=1),
                Conv2d(base_channels * 4, base_channels * 4, 3, 1, padding=1))

        self.out1 = nn.Sequential(
                Conv2d(base_channels * 4, base_channels * 1, 1),
                DCN(in_channels=base_channels * 1, out_channels=base_channels * 1, kernel_size=3, stride=1, padding=1),
...
if __name__ == '__main__':
    ...
    model = Conv2d(3, 5, 5, stride=2, padding=2)
    out = model(input)
    print('test')
